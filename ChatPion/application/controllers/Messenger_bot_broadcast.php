...
class Messenger_bot_broadcast extends Home
{
    ...
    {   
        ...
        if(!check_module_action_access($module_id=275,$actions=3,'check')){
            echo '0';
            exit();
        }
    ...
    public function otn_force_reprocess_campaign()
    {
        ...
        if(!check_module_action_access($module_id=275,$actions=2,'check')){
            echo '0';
            exit();
        }
    ...
    public function otn_create_subscriber_broadcast_campaign()
    {
        ...
        $data['page_info'] = $this->basic->get_data("facebook_rx_fb_page_info",$where,$select='',$join='',$limit='',$start=NULL,$order_by='page_name ASC');
    ...
    public function otn_subscriber_bulk_broadcast_add_action()
    {
        $this->ajax_check();
        $this->csrf_token_check();
        check_module_action_access($module_id=275,$actions=1);        
    ...
    {   
        ...
        $this->ajax_check();
        check_module_action_access($module_id=275,$actions=2);
    ...
    public function rcn_subscriber_broadcast_campaign_data()
    { 
        $this->ajax_check();
        if($this->session->userdata('user_type') != 'Admin' && !in_array(335,$this->module_access)) exit();
        ...
        foreach($info as $key => $value) 
        {
            ...
            {                
                $edit_url = site_url('messenger_bot_broadcast/rcn_edit_subscriber_broadcast_campaign/'.$info[$key]['id']);
                $info[$key]['edit'] =  "<a class='btn btn-circle btn-outline-warning' data-toggle='tooltip' title='".$this->lang->line("Edit Campaign")."' href='".$edit_url."'><i class='fas fa-edit'></i></a>";
            }
    ...
    public function rcn_subscriber_delete_campaign()
    {   
        $this->ajax_check();
        $this->csrf_token_check();
    ...
    public function rcn_force_reprocess_campaign()
    {
        $this->ajax_check();
        $this->csrf_token_check();
        if(!check_module_action_access($module_id=335,$actions=2,'check')){
            echo '0';
            exit();
        }
    ...
    public function rcn_restart_campaign()
    {
        $this->ajax_check();
        if(!check_module_action_access($module_id=335,$actions=2,'check')){
            echo '0';
    ...
    public function rcn_ajax_campaign_pause()
    {
        $this->ajax_check();
        $this->csrf_token_check();
        if(!check_module_action_access($module_id=335,$actions=2,'check')){
    ...
    public function rcn_ajax_campaign_play()
    {
        $this->ajax_check();
        $this->csrf_token_check();
        if(!check_module_action_access($module_id=335,$actions=2,'check')){
    ...
    public function rcn_campaign_sent_status()
    {
        $this->ajax_check();
        $this->csrf_token_check();
        if($this->session->userdata('user_type') != 'Admin' && !in_array(335,$this->module_access)) exit();
    ...
    public function rcn_campaign_sent_status_data()
    { 
        $this->ajax_check();
        $this->csrf_token_check();
    ...
    private function rcn_get_broadcast_tags()
    {
      $old_tags = array
      (
        "ACCOUNT_UPDATE" => "ACCOUNT_UPDATE",
    ...
    public function rcn_create_subscriber_broadcast_campaign()
    {
        ...
        $data['body'] = 'messenger_tools/rcn_manager/subscriber_bulk_broadcast_add';
        $data['page_title'] = $this->lang->line('Add RCN Subscriber Broadcast');  
    ...
    public function rcn_subscriber_bulk_broadcast_add_action()
    {      
        if(function_exists('ini_set')){
            ini_set('memory_limit', '-1');
        }
        ...
        $campaign_message_send=$bot_message;
        $campaign_message_send["recipient"]=array("notification_messages_token"=>"PUT_OTN_TOKEN");
        ...
        {
            $otn_postback_str = implode("','", $otn_postback_ids);
            $sql="SELECT `messenger_bot_subscriber`.*,`otn_optin_subscriber`.otn_token FROM otn_optin_subscriber LEFT JOIN `messenger_bot_subscriber` ON `otn_optin_subscriber`.`subscriber_id`=`messenger_bot_subscriber`.`subscribe_id` LEFT JOIN `messenger_bot_subscribers_label` ON `messenger_bot_subscribers_label`.`subscriber_table_id`=`messenger_bot_subscriber`.`id` WHERE ".$sql_part." ".$sql_part2." ".$sql_part3." ".$promo_sql." user_id = ".$this->user_id." AND unavailable = '0' AND is_bot_subscriber='1' AND messenger_bot_subscriber.page_table_id = {$page_table_id} AND `otn_id` IN('".$otn_postback_str."') GROUP BY otn_optin_subscriber.subscriber_id";
        }
        else
            $sql="SELECT `messenger_bot_subscriber`.*,`otn_optin_subscriber`.otn_token FROM otn_optin_subscriber LEFT JOIN `messenger_bot_subscriber` ON `otn_optin_subscriber`.`subscriber_id`=`messenger_bot_subscriber`.`subscribe_id` LEFT JOIN `messenger_bot_subscribers_label` ON `messenger_bot_subscribers_label`.`subscriber_table_id`=`messenger_bot_subscriber`.`id` WHERE ".$sql_part." ".$sql_part2." ".$sql_part3." ".$promo_sql." user_id = ".$this->user_id." AND unavailable = '0' AND is_bot_subscriber='1' AND messenger_bot_subscriber.page_table_id = {$page_table_id} GROUP BY otn_optin_subscriber.subscriber_id";
        ...
        $status=$this->_check_usage($module_id=335,$request=$total_thread);
        if($status=="2")  //monthly limit is exceeded, can not send another ,message this month
        {
            echo json_encode(array('status'=>'0','message'=>$this->lang->line("Sorry, your bulk to send subscriber message is exceeded.")));
            exit();
        ...
        {
            $campaign_id= $this->db->insert_id();
            $this->_insert_usage_log($module_id=335,$request=$total_thread);
    ...
    {  
        ...
        $data['body'] = 'messenger_tools/rcn_manager/subscriber_bulk_broadcast_edit';
        $data['page_title'] = $this->lang->line('Edit OTN Subscriber Broadcast');  
    ...
    public function rcn_subscriber_bulk_broadcast_edit_action()
    {   
        ...
        $this->ajax_check();
       check_module_action_access($module_id=335,$actions=2);
        ...
        $this->basic->delete_data("messenger_bot_broadcast_serial_send",array("campaign_id"=>$xid,"user_id"=>$this->user_id));
        $this->_delete_usage_log(335,$total_thread);
        $this->db->trans_complete();
        if($this->db->trans_status() === false) 
        echo json_encode(array('status'=>'0','message'=>$this->lang->line('Something went wrong, please try again.')));
        else 
