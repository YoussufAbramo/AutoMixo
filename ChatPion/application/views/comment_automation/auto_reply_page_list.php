...
<section class="section">
	<div class="section-header">
        ...
        <div class="breadcrumb-item active"><a href="<?php echo base_url('comment_automation/comment_growth_tools/'.$this->using_media_type); ?>"><?php echo $this->lang->line('Comment Growth Tools'); ?></a></div>
...
<script>
	...
	$(document).ready(function(){
		...
		$(document).on('click','.save_button',function(){
			...
			var request_url = base_url+"comment_automation/ajax_autoreply_submit";
					...
					<div id="new_template_section">
						...
						<div class="row" style="padding: 10px 20px 10px 20px;">
							...
							<div class="col-12" id="ai_message_div" style="display: none;">
								...
								<div>                      
								  <select class="form-group private_reply_postback" id="ai_message_private" name="ai_message_private">
								    <option><?php echo $this->lang->line('Please select a page first to see the message templates.'); ?></option>
								  </select>
...
<script type="text/javascript">
			...
			if (willDelete) 
			{
				...
				$.ajax({
					...
					success:function(response){ 
						...
						{
							var span = document.createElement("span");
							span.innerHTML = '<?php echo $this->lang->line("You do not have permission to access this module."); ?>';
							swal({ title:'<?php echo $this->lang->line("Error!"); ?>', content:span,icon:'error'});
						}
					}
				});
	...
	$(document).on('click','.get_post_analytics',function(e){
	  e.preventDefault();
	    ...
	    $("#all_comments_modal_contents").html('<div class="text-center waiting p-4"><i class="fas fa-spinner fa-spin blue text-center font_size_60px"></i></div');
	  	...
	  	$.ajax({
	  		url: base_url+'comment_automation/get_post_analytics',
	  		type: 'post',
	  		data: {page_table_id:page_table_id, post_id:post_id},
	  		success:function(response) {
	  			$("#all_comments_modal_contents").html(response);
	...
	$(document).on('click','.delete_single_comment',function(e){
	    e.preventDefault();
	  	var page_table_id = $(this).attr("page_table_id");
	  	var comment_id = $(this).attr("comment_id");

		var mes='<?php echo $this->lang->line("Do you really want to delete this comment?");?>';  
		$(this).addClass('btn-progress');
		swal({
			context: this,
			title: "<?php echo $this->lang->line("Are you sure?");?>",
		...
		.then((willDelete) => 
		{
			...
			{
				$.ajax({
					url: base_url+'comment_automation/delete_single_comment',
					type: 'post',
					dataType: 'JSON',
					data: {page_table_id:page_table_id, comment_id:comment_id},
					success:function(response) {
						$(this).removeClass('btn-progress').addClass('btn-danger');
						$("#"+comment_id).addClass('d-none');
						if(response.status == 1)  
						{
	...
	$(document).on('click','.block_single_commenter',function(e){
	    e.preventDefault();
	  	var page_table_id = $(this).attr("page_table_id");
	  	var commenter_id = $(this).attr("commenter_id");

		var mes='<?php echo $this->lang->line("Do you really want to block this person from commenting on your post?");?>';  
		$(this).addClass('btn-progress');
		swal({
			context: this,
			title: "<?php echo $this->lang->line("Are you sure?");?>",
		...
		.then((willDelete) => 
		{
			...
			{
				$.ajax({
					url: base_url+'comment_automation/block_person_from_commenting',
					type: 'post',
					dataType: 'JSON',
					data: {page_table_id:page_table_id, commenter_id:commenter_id},
					success:function(response) {
						$(this).removeClass('btn-progress').addClass('btn-danger');
						if(response.status == 1)  
						{
							// $("#all_comments_modal").modal('hide');
							iziToast.success({title: '',message: response.message,position: 'bottomRight'});
						}
						else iziToast.error({title: '',message: response.message,position: 'bottomRight'});
					}
	...
	$(document).on('click','.reply_single_comment',function(e){
		e.preventDefault();
		var page_table_id = $(this).attr("page_table_id");
	  	var comment_id = $(this).attr("comment_id");
		$("#comment-"+comment_id).removeClass('d-none').addClass('d-block');
	});

	$(document).on('click','.cancel_single_reply',function(e){
		e.preventDefault();
	  	var comment_id = $(this).attr("comment_id");
		$("#comment-"+comment_id).removeClass('d-block').addClass('d-none');
	});


	$(document).on('click','.send_single_reply',function(e){
	    e.preventDefault();
	  	var page_table_id = $(this).attr("page_table_id");
	  	var comment_id = $(this).attr("comment_id");
		var message = $("#textarea-"+comment_id).val();
		if(message == '')
		{
		...
		$.ajax({
			url: base_url+'comment_automation/reply_single_comment',
			type: 'post',
			dataType: 'JSON',
			data: {page_table_id:page_table_id, comment_id:comment_id, message:message},
			success:function(response) {
				$(this).removeClass('btn-progress').addClass('btn-info');
				$("#comment-"+comment_id).addClass('d-none').removeClass('d-block');
	...
	$('#auto_reply_message_modal').on('shown.bs.modal', function() {
	    $(document).off('focusin.modal');
	}); 

	$('#edit_auto_reply_message_modal').on('shown.bs.modal', function() {
	    $(document).off('focusin.modal');
	}); 
