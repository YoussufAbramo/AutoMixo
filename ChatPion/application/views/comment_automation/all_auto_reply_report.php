	...
	$(document).on('click','.renew_campaign',function(){		
		...
		$.ajax({
			...
			success:function(response){
				if(response == 'success')
				{
					iziToast.success({title: '',message: '<?php echo $this->lang->line("Campaign has been renewed successfully."); ?>',position: 'bottomRight'});
					table.draw();
				}
				if(response == 'error')
				{
	...
	$(document).on('click','.edit_reply_info',function(){
		...
		$.ajax({
		  ...
		  success:function(response){
		  	...
		  	$("#edit_private_message_offensive_words").val(response.private_message_offensive_words);
    ...
    <div class="modal-dialog modal-mega">
        <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title"><i class="fas fa-reply-all"></i> <?php echo $this->lang->line("Report of Auto Reply");?>
              	<small>
              	(
              	<?php 
