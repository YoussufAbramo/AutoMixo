...
{ ?>
        ...
        <div class="card-header padding-left-10 padding-right-10">
          ...
          <div class="col-4 padding-0">
            ...
            <ul class="dropdown-menu dropdown-menu-sm dropdown-menu-right" style="width: 250px;">
              ...
              <li><a class="dropdown-item has-icon import_flow" media_type="<?php echo $media_type; ?>" table_id="" href="#"><i class="fas fa-file-import"></i> <?php echo $this->lang->line("Import flow");?></a></li>
              <?php endif; ?>
           </ul>
          </div>
        </div>
                                    ...
                                    <div class="section">                
                                     ...
                                     <div class=""  id="delay_con2">
                                       <div class="form-group">
                                         ...
                                         <a href="#" target="_BLANK" class="btn btn-outline-warning float-right btn-sm" id="getstarted_button_edit_url"><i class="far fa-edit"></i> <?php echo $this->lang->line("edit get started reply");?></a>   
                                       </div>
                                     </div>
                                    <br><br>
                                  </div>
  ...
  <div class="modal fade" data-backdrop="static" data-keyboard="false" id="flowImportModal">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h3 class="modal-title"><?php echo $this->lang->line('Upload Text File'); ?></h3>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form id="flowImportForm" enctype="multipart/form-data" >
      <div class="modal-body">
          <div class="form-group">
              <label for=""><?php echo $this->lang->line('Select a text file'); ?></label>
              <div>
                  <input type="file" name="file" id="file" class="form-control" accept=".txt">
                  <span class="small text-muted"> <?php echo $this->lang->line('File type support : only .txt'); ?> </span>
              </div>
          </div>
      ...
      <div class="modal-footer">
        ...
        <button type="button" id="saveImport"  class="btn btn-primary"><?php echo $this->lang->line('Save changes'); ?></button>
      </div>
      </form>
    </div>
  </div>
...
<script type="text/javascript">
  ...
  $(document).ready(function(){
    ...
    $('.import_flow').click(function() {
        $('#file').val('');
        $('#flowImportModal').modal('show');
    });
    ...
    $('#saveImport').click(function() {
        ...
        if (file) {
            ...
            $.ajax({
                url: base_url+'messenger_bot/import_flow_data?media_type='+selected_global_media_type, // Replace with your controller endpoint
                type: 'POST',
                data: formData,
                contentType: false,
                processData: false,
                success: function(response) {
                  ...
                  {
                    swal('<?php echo $this->lang->line("Error"); ?>', response.message, 'error').then((value) => {
                          $(".page_list_item.active").click();
                      });
                  }
                }
          ...
          swal('<?php echo $this->lang->line("Warning"); ?>', '<?php echo $this->lang->line("Please attatch a file"); ?>', 'error').then((value) => {
            $(".page_list_item.active").click();
          });
        }
    });
        ...
        .then((willreset) => {
            if(willreset) 
            {
                ...
                $.ajax({
                    context: this,
                    type:'POST',
                    url: base_url + "social_accounts/reset_action_settings?media_type="+selected_global_media_type,
                    dataType: 'json',
                    data: {page_table_id},
                    success:function(response){ 
                    if(response.status == 1)
    ...
    $(document).on('click','#export_bot_submit',function(e){
      ...
      $.ajax({
            ...
            { 
              ...
              var span = document.createElement("span");
              span.innerHTML = success_message;
              if(response.status=='1') swal({ title:'<?php echo $this->lang->line("Export Status"); ?>', content:span,icon:'success'});
              else swal({ title:'<?php echo $this->lang->line("Export Status"); ?>', content:span,icon:'error'});
            }
      });
