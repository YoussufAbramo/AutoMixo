...
class Messenger_bot extends Home
{
    ...
    public function send_message_bot_reply($value='',$sender_id='',$subscriber_info=[],$page_id='',$user_reference_id="",$answer="",$fb_message_id="",$message_type=""){
        ...
        $main_message_array=json_decode($message_str,true);

        if(isset($main_message_array['out'])){
           $message_str =  $this->process_message_condition($message_str,$subscriber_info,$subscriber_info[0]['subscribe_id']);
        }
            ...
            foreach($message_array as $msg)
            {
                ...
                if($template_type_file_track=='User_Input_Flow'){
                    ...
                    if($question_id=="" && $invalid_answer==false){
                        ...
                        if(isset($form_submit_email_template[0]) && $form_submit_email_template[0]['subject'] != '' && $form_submit_email_template[0]['message'] != '') {
                            ...
                            $form_data_mail = json_decode($form_data_json,true);

                            $str_html = '';
                            if(!empty($form_data_mail)) {
                                $str_html.= '<table cellpadding="0" cellspacing="0" width="640" align="center" border="1">
                                        <tr>
                                              <th scope="col" style="padding:5px; font-family: Arial,sans-serif; font-size: 16px; line-height:20px;line-height:30px">Question</th>
                                              <th scope="col" style="padding:5px; font-family: Arial,sans-serif; font-size: 16px; line-height:20px;line-height:30px">Answer</th>
                                           </tr>';

                                $i=0;
                                foreach($form_data_mail as $body_value) {
                                    $str_html.= '<tr>
                                            <td valign="top" style="padding:5px 10px; font-family: Arial,sans-serif; font-size: 14px; line-height:20px;">'.$body_value['question'].'</td>
                                            <td valign="top" style="padding:5px 10px; font-family: Arial,sans-serif; font-size: 14px; line-height:20px;">'.$body_value['answer'].'</td>
                                            </tr>';
                                }
                                $str_html .= '</table>';
                            }

                             $message = str_replace(array('#APP_NAME#','#APP_URL#','#SUBSCRIBER_NAME#','#FLOW_NAME#','#FLOW_DATA#'),array($mask,$url,$subscriber_name,$form_title,$str_html),$form_submit_email_template[0]['message']);
                ...
                if((isset($subscriber_info[0]['status']) && $subscriber_info[0]['status']=="1") || $user_reference_id!="")
                {
                 ...
                 if(isset($reply_response['error']['message'])){
                    ...
                    $message_id = $reply_response['message_id'] ?? null;
    ...
    public function webhook_callback_main()
    {
        ...
        // $response_raw = '{"object":"page","entry":[{"id":"218002968934338","time":1704179334047,"messaging":[{"sender":{"id":"1618664901587376"},"recipient":{"id":"218002968934338"},"timestamp":1704179333608,"message":{"mid":"m_lN7LCjHCUFwslTbdDzMADywznpis2kHYhN4biNyl-qJUhNrSNc__FAD-bOMIjaU1Jfd3zjjsmKJAB6aM1BSD0g","text":"m-assign"}}]}]}';

        $response = json_decode($response_raw,TRUE);
        if(isset($response['entry']['0']['messaging'][0]['delivery'])) exit();
        if(isset($response['entry']['0']['messaging'][0]['read'])) exit;

        if(isset($response['object']) && $response['object']=="instagram") $social_media_type="ig";
        else $social_media_type="fb";
        ...
        if($social_media_type=="ig"){
            $where = array("where"=>array("instagram_business_account_id"=>$page_id,"bot_enabled"=>"1"));
        ...
        $insert_livechat_data = [
            'subscriber_id' => $sender_id ?? null,
            'fb_page_id' => $page_id ?? null,
            'sender' => 'user',
            'platform'=>$social_media_type??'fb',
            'message_content' => $response_raw,
            'conversation_time' =>date("Y-m-d H:i:s"),
        ];
        $this->insert_livechat_data($insert_livechat_data);
        ...
        && !isset($response['entry'][0]['messaging'][0]['message']['reply_to']['story']))
        {
            ...
            foreach ($messenger_bot_info as $key => $value) {
                ...
                foreach ($cam_keywords_array as $cam_keywords)
                {
                    ...
                    if($value['trigger_matching_type'] == 'exact')
                    {
                        $search_array = [];
                    ...
                    if($matches !== FALSE)
                    {
                        $this->send_message_bot_reply($value,$sender_id,$subscriber_info,$page_id,"",$messages);

                        // Assign drip campaign 
                        $drip_campaign_id=$value['drip_campaign_id'] ?? 0;
                        if(isset($drip_campaign_id) && $drip_campaign_id>0){
                            $drip_type="custom";
                        ...
                        if(isset($value['team_assign_user_id']) && $value['team_assign_user_id']!='')
                          $this->assign_conversation_to_team_member($sender_id, $value['team_assign_user_id'], $subscriber_info[0]['page_table_id'], $value['team_assign_role_id'],$social_media_type);


                        // Remove drip campaign
                        $remove_drip_campaign_id= $value['remove_sequence_campaign_id'] ?? 0; 

                        if(isset($remove_drip_campaign_id) && $remove_drip_campaign_id>0){
                            $drip_type="custom";
                            $this->remove_drip_messaging_id($drip_type,"0",$subscriber_info[0]['page_table_id'],$subscriber_info[0]['subscribe_id'],$remove_drip_campaign_id);
                        }
                        ...
                        if(!empty($label_ids))
                            $this->multiple_assign_label($sender_id,$page_id,$label_ids,$social_media_type,$subscriber_info[0]['id']);
                        // Remove labels 
                        $remove_label_ids=$value['remove_label_ids'] ?? "";
                        if(!empty($remove_label_ids))
                            $this->multiple_remove_label($sender_id,$page_id,$remove_label_ids,$social_media_type,$subscriber_info[0]['id']);
            ...
            {
                $this->send_message_bot_reply($messenger_bot_info[0],$sender_id,$subscriber_info,$page_id,"",$messages);
                //update Subscriber Last Interaction time.
                $this->update_subscriber_last_interaction($sender_id,$currenTime,$subsciber_broadcast_unavailable);
                die();
            }
        ...
        elseif(isset($response['entry'][0]['messaging'][0]['optin'])) //Optins from Send to messengers
        {
            ...
            if($is_one_time_notification!='one_time_notif_req' && $is_one_time_notification != 'notification_messages')
            {
            ...
            elseif ($response['entry'][0]['messaging'][0]['optin']['type']=='one_time_notif_req' || $response['entry'][0]['messaging'][0]['optin']['type']=='notification_messages') {
                ...
                if($response['entry'][0]['messaging'][0]['optin']['type']=='one_time_notif_req')
                    $otn_token= $response['entry'][0]['messaging'][0]['optin']['one_time_notif_token'];
                else
                    $otn_token= $response['entry'][0]['messaging'][0]['optin']['notification_messages_token'];
            ...
            if(isset($messenger_bot_info[0]) && !empty($messenger_bot_info)){
                ...
                if($is_one_time_notification=='one_time_notif_req'){
                    ...
                    $this->assign_drip_messaging_id($drip_type,$plugin_auto_id="0",$subscriber_info[0]['page_table_id'],$subscriber_info[0]['subscribe_id']);
                }
        ...
        elseif(
            (isset($response['entry'][0]['messaging'][0]['postback']['referral']['type']) && $response['entry'][0]['messaging'][0]['postback']['referral']['type']=="OPEN_THREAD" && isset($response['entry'][0]['messaging'][0]['postback']['referral']['ref'])) ||
            ...
            if($enable_mark_seen && $custom_chat_user_ref=="") // mark ass seen action
              ...
              if(isset($messenger_bot_info[0]) && !empty($messenger_bot_info)){
                ...
                if(!empty($label_ids) && isset($subscriber_info[0]['id'])){
                    //DEPRECATED FUNCTION FOR QUICK BROADCAST
                    $this->multiple_assign_label($sender_id,$page_id,$label_ids,$social_media_type,$subscriber_info[0]['id']);
                 ...
                 //update Subscriber Last Interaction time.
                if(isset($subsciber_broadcast_unavailable))
                    $this->update_subscriber_last_interaction($sender_id,$currenTime,$subsciber_broadcast_unavailable);
        ...
        elseif (isset($response['entry'][0]['messaging'][0]['message']['quick_reply'])) 
        {
            //quick_reply
            ...
            if(isset($messenger_bot_info[0]) && !empty($messenger_bot_info)){
                ...
                // Remove drip campaign
                $remove_drip_campaign_id= $messenger_bot_info[0]['remove_sequence_campaign_id'] ?? 0; 
                if(isset($remove_drip_campaign_id) && $remove_drip_campaign_id>0){
                    $drip_type="custom";
                    $this->remove_drip_messaging_id($drip_type,"0",$subscriber_info[0]['page_table_id'],$subscriber_info[0]['subscribe_id'],$remove_drip_campaign_id);
                }
                ...
                // Remove labels 
                $remove_label_ids=$messenger_bot_info[0]['remove_label_ids'] ?? "";
                if(!empty($remove_label_ids))
                    $this->multiple_remove_label($sender_id,$page_id,$remove_label_ids,$social_media_type,$subscriber_info[0]['id']);
        ...
        elseif(isset($response['entry'][0]['messaging'][0]['postback'])) 
        {
            //Clicking on Payload Button like Start Chatting
            POST_BACK_BLOCK:
        ...
        elseif(
            isset($response['entry'][0]['messaging'][0]['message']['reply_to']['story']['id']) &&
        ...
        elseif(
            isset($response['entry'][0]['messaging'][0]['message']['is_deleted']) &&
    ...
    public function import_flow_data()
	{
        ...
        if(isset($_FILES['file']) && !empty($_FILES['file']['tmp_name'])){
            ...
            $final_data = $this->updateArrayValues($jsonData);
            ...
            if($this->basic->insert_data('visual_flow_builder_campaign',$insert_data))
            {
                $response['status'] = '1';
                $response['message'] = $this->lang->line('Flow successfully Imported');   
                echo json_encode($response);
            }
            else{
                $response['status'] = '0';
                $response['message'] = $this->lang->line('Flow Import Failed');  
                echo json_encode($response);
            }
        }
    ...
    public function get_page_details()
    {
        ...
        if($media_type == 'ig')
            ...
            $postback_data=$this->basic->get_data("messenger_bot_postback",array("where"=>array("page_id"=>$page_table_id,"is_template"=>"1",'template_for'=>'reply_message','media_type'=>'fb')),'','','',$start=NULL,'id DESC');
        ...
        $murl = base_url('messenger_bot/persistent_menu_list/').$page_table_id.'/1?media_type='.$media_type;
        ...
        $bot_reply_settings_url = base_url("messenger_bot/bot_settings/").$page_info[0]['id']."/1?media_type=".$media_type;
        $postback_manager_url = base_url("messenger_bot/template_manager/").$page_info[0]['id']."/1?media_type=".$media_type;
        ...
        $bot_flow_settings_url = base_url('visual_flow_builder/flowbuilder_manager/').$page_info[0]['id']."/1?media_type=".$media_type;

        $nomatch_reply_found_val = $page_info[0]['no_match_found_reply'];
        $chatwith_human_email = $page_info[0]['chat_human_email'];
        if($media_type == "ig") {
            $bot_reply_settings_url = base_url("messenger_bot/ig_bot_settings/").$page_info[0]['id']."/1?media_type=".$media_type;
        ...
        <div class="card main_card">
          ...
          <div class="card-body padding-10">
            <div class="row">
              <div class="col-12">
                <div class="card card-large-icons card-condensed active">
                  ...
                  <div class="card-body">
                    <h4>'.$this->lang->line("Bot Reply").'</h4>                    
            ...
            $rest_action_str = '
                ...
                <div class="col-12 col-md-6 offset-md-3">
                    <a href="#" class="pointer reset_action_settings" data-height="795" page_table_id="'.$page_info[0]['id'].'">
                      <div class="card card-large-icons card-condensed bg-body" style="margin-top:50px !important;">
                        <div class="card-icon" style="width:70px !important;">
                          <i class="fa fa-refresh text-danger" aria-hidden="true"></i>
                        </div>
                        <div class="card-body">
                          <h4>'.$this->lang->line("Reset all action button settings to default.").'</h4>
                        </div>
                      </div>
                    </a>
                </div>';
            ...
            $action_buttons_str .= $rest_action_str;
         ...
         if($this->session->userdata('user_type') == 'Admin' || in_array(197,$this->module_access)) :
            ...
            if($media_type=="ig") $persistent_field="ig_persistent_enabled"; else $persistent_field="persistent_enabled";
            if($page_info[0][$persistent_field]=='1') $pm_str='<small class="badge badge-status green">'.$this->lang->line("Published").'</small>';
            $middle_column_content .='
              <div class="col-12">
                <div class="card card-large-icons card-condensed">
                  <div class="card-icon">
        ...
        if($this->recurring_notification_exist) :
            ...
            $rcn_view_loader = base_url("messenger_bot/rcn_template_manager/").$page_info[0]['id'].'/1';
            $middle_column_content .= '
            <div class="row '.$hide_sections.'">
                <div class="col-12">
                    <div class="card card-large-icons card-condensed">
                      ...
                      <div class="card-body">
                        <h4>'.$this->lang->line("RCN post-back manager").'</h4>
                        <a class="card-cta iframed" href="'.$rcn_view_loader.'" data-page-id="'.$page_info[0]['id'].'" data-height="795" class="card-cta" id="otn_postback_settings">'.$this->lang->line("Change Settings").'
                        </a>
                      </div>
                    </div>
                </div>
                    ...
                    <div class="col-12">
                        <div class="card card-large-icons card-condensed">
                          ...
                          <div class="card-body">
                            ...
                            <a target="_BLANK" href="'.base_url('messenger_bot/saved_templates/'.$media_type).'" class="card-cta">'.$this->lang->line("Change Settings").'</a>
                          </div>
                        </div>
                    </div>
                </div>
    ...
    {
        if(!check_module_action_access($module_id=199,$actions=2,'check')){
    ...
    public function ig_bot_settings($page_auto_id='0',$iframe='0')
    {
        check_module_access($module_id=320);
        ...
        if(!isset($page_info[0]))
        redirect('messenger_bot/bot_list?media_type=ig', 'location');
    ...
    public function get_postback_for_persistent_menu()
    {
        ...
        $postback_data=$this->basic->get_data("messenger_bot_postback",array("where"=>array("page_id"=>$page_id,'is_template'=>'1','template_for'=>'reply_message',"media_type"=>$media_type)),'','','',$start=NULL,$order_by);
        $push_postback="";
        foreach ($postback_data as $key => $value)
        {
    ...
    public function template_manager($page_auto_id='0',$iframe='0')
    {
        ...
        $data['iframe'] = $iframe;
        $where = array('where'=>array('facebook_rx_fb_page_info.user_id'=>$this->user_id,'bot_enabled'=>'1'));
         if(!empty($this->team_allowed_pages)){
            $where['where_in'] = array("facebook_rx_fb_page_info.id"=>$this->team_allowed_pages);
        }
        ...
        $page_info = $this->basic->get_data('facebook_rx_fb_page_info',$where,array('facebook_rx_fb_page_info.id','page_name','name'),$join);
        ...
        if(isset($media_type) && $media_type == "ig") {
            ...
            $ig_where = array('where'=>array('facebook_rx_fb_page_info.user_id'=>$this->user_id,'bot_enabled'=>'1','has_instagram'=>'1'));
            if(!empty($this->team_allowed_pages)){
                $ig_where['where_in'] = array("facebook_rx_fb_page_info.id"=>$this->team_allowed_pages);
            }
            $ig_page_info = $this->basic->get_data('facebook_rx_fb_page_info',$ig_where,array('facebook_rx_fb_page_info.id','page_name','name','insta_username'),$join);
    ...
    public function create_new_template($is_iframe="0",$default_page="",$default_child_postback_id="",$media_type='fb')
    {
        ...
        $join = array('facebook_rx_fb_user_info'=>'facebook_rx_fb_page_info.facebook_rx_fb_user_info_id=facebook_rx_fb_user_info.id,left');
        if($media_type == 'ig'){
            $where = array('where'=>array('facebook_rx_fb_page_info.user_id'=>$this->user_id,'bot_enabled'=>'1','has_instagram'=>'1'));
            if(!empty($this->team_allowed_pages)){
                $where['where_in'] = array("facebook_rx_fb_page_info.id"=>$this->team_allowed_pages);
            }
            $page_info = $this->basic->get_data('facebook_rx_fb_page_info',$where,array('facebook_rx_fb_page_info.id','page_name','name','insta_username'),$join);
        }
        else{
            $ig_where = array('where'=>array('facebook_rx_fb_page_info.user_id'=>$this->user_id,'bot_enabled'=>'1'));
            if(!empty($this->team_allowed_pages)){
                $ig_where['where_in'] = array("facebook_rx_fb_page_info.id"=>$this->team_allowed_pages);
            }
            $page_info = $this->basic->get_data('facebook_rx_fb_page_info',$ig_where,array('facebook_rx_fb_page_info.id','page_name','name'),$join);
        }
        ...
        $postback_id_list = $this->basic->get_data('messenger_bot_postback',array('where'=>array('user_id'=>$this->user_id,'media_type'=>$media_type)));
    ...
    public function edit_template($postback_table_id=0,$iframe='0',$is_default='0',$media_type='fb')
    {
        ...
        $bot_info = $this->basic->get_data($table_name, $where_bot);
        if(empty($bot_info)) redirect('messenger_bot/template_manager?media_type='.$media_type, 'location');
    ...
    {
        check_module_action_access($module_id=199,$actions=2);
        if(!$_POST) exit();
    ...
    {
        check_module_access($module_id=257);
        check_module_action_access($module_id=257,$actions=4);
            ...
            foreach($input_flow as $flow_info){
                ...
                foreach($questions_list as $q_list){
                    ...
                    foreach($q_list as $key=>$value){
                        ...
                        if($key=="custom_field_id" && $value>0 && isset($custom_field_id_change_log[$value]))
         ...
         if($this->db->table_exists('messenger_bot_drip_campaign')) {
            ...
            foreach($sequence_campaign_information as $sequence_campaign_list){
                ...
                foreach($message_content as $dkey=>$dvalue){
                    if(isset($new_postback_id_information[$dvalue])) {
    ...
    private function make_tree($get_started_data_copy,$is_get_started=1,$page_table_id=0) // 0 = keyword, 1=get started, 2 = no match
    {
        ...
        $this->postback_array=$this->set_nest_easy($get_started_level,$postback_array);
    ...
    {
        check_module_action_access($module_id=199,$actions=3);
        ...
        echo json_encode(array('success'=>1,'message'=>$this->lang->line("Bot has been deleted successfully.")));
   ...
   //=============================DOMAIN WHITELIST================================
    ...
    {
        if(!check_module_action_access($module_id=199,$actions=3,'check')){
        ...
        else $this->lang->line('Something went wrong, please try again later.');
    }
    ...
    {
        check_module_action_access($module_id=199,$actions=1);
        $this->ajax_check();
   ...
   //=============================DOMAIN WHITELIST================================
    ...
    public function remove_persistent_menu_locale($auto_id=0,$page_auto_id=0,$iframe=0)
    {
        ...
        check_module_action_access($module_id=197,$actions=3);
        $media_type = $this->using_media_type;
    ...
    public function remove_persistent_menu($page_auto_id=0,$iframe=0)
    {
        ...
        $response=$this->fb_rx_login->delete_persistent_menu($page_access_token,$media_type);
        if(!isset($response['error']))
        {
            if($media_type=="fb")
                $update_data=array("persistent_enabled"=>'0');
            else
                $update_data=array("ig_persistent_enabled"=>'0');

            $this->basic->update_data('facebook_rx_fb_page_info',array("id"=>$page_auto_id,'user_id'=>$this->user_id),$update_data);
            $this->basic->delete_data('messenger_bot_persistent_menu',array("page_id"=>$page_auto_id,'user_id'=>$this->user_id,"media_type"=>$media_type));
    ...
    public function publish_persistent_menu($page_auto_id=0,$iframe=0)
    {
        ...
        check_module_action_access($module_id=197,$actions=2);
        $page_info=$this->basic->get_data("facebook_rx_fb_page_info",array("where"=>array("id"=>$page_auto_id,'user_id'=>$this->user_id)));
        if(!isset($page_info[0])) exit();
        $media_type = $this->using_media_type;
        $page_access_token=$page_info[0]['page_access_token'];
        ...
        $menu_data=$this->basic->get_data("messenger_bot_persistent_menu",array("where"=>array("page_id"=>$page_auto_id,"user_id"=>$this->user_id,"media_type"=>$media_type)));
        foreach ($menu_data as $key => $value)
        {
            $temp=json_decode($value["item_json"],true);
        ...
        $response=$this->fb_rx_login->add_persistent_menu($page_access_token,$json,$media_type);
        ...
        if(!isset($response['error']))
        {
            ...
            if($media_type=="fb")
                $update_data=array("persistent_enabled"=>'1');
            else
                $update_data=array("ig_persistent_enabled"=>'1');
    ...
    {
        check_module_action_access($module_id=197,$actions=1);
    ...
    public function create_persistent_menu_action()
    {
        ...
        if($this->basic->is_exist("messenger_bot_persistent_menu",array("page_id"=>$page_table_id,"locale"=>$locale,"media_type"=>$media_type)))
        {
        ...
        else // For ig composer input disable option is unavailable.
    ...
    public function edit_persistent_menu($id=0,$iframe=0)
    {
        ...
        $data['page_info'] = isset($page_info[0]) ? $page_info[0] : array();
        $started_button_enabled = isset($page_info[0]["started_button_enabled"])?$page_info[0]["started_button_enabled"]:"0";
        $postback_id_list = $this->basic->get_data('messenger_bot_postback',array('where'=>array('user_id'=>$this->user_id,'page_id'=>$page_auto_id,"media_type"=>$media_type)));
        $data['postback_ids'] = $postback_id_list;
        $data['page_auto_id'] = $page_auto_id;
        $data['started_button_enabled'] = $started_button_enabled;
        $data['locale']=$this->sdk_locale();
    ...
    public function edit_persistent_menu_action()
    {
        ...
        $media_type = $this->using_media_type;;
        if($this->basic->is_exist("messenger_bot_persistent_menu",array("page_id"=>$page_table_id,"locale"=>$locale,"media_type"=>$media_type,"id!="=>$auto_id)))
        {
            echo json_encode(array('status'=>'0','message'=>$this->lang->line("persistent menu is already exists for this locale.")));
            exit();
        }
        ...
        else // For If it always 0 as IG doesn't support this feature.
            $composer_input_disabled=0;
    ...
    private function set_nest_easy($get_started_level,$postback_array=array())
    {
...
'.json_encode($json_value).'
        ...
        */
        public function saved_templates($media_type='fb')
        {
            ...
            for ($i=0; $i < count($info) ; $i++) {
                if($info[$i]['template_category_id'] > 0) {
                    ...
                    $info[$i]['category_name'] = $get_category_name[0]['category_name'] ?? $this->lang->line("No category");
                } else {
        ...
        public function delete_template_category()
        {
            ...
            $cat_id = $this->input->post("table_id",true);
            if($cat_id == "" || $cat_id == '0') exit;

            $this->basic->delete_data("messenger_bot_template_category",['id'=>$cat_id]);
            $this->basic->update_data("messenger_bot_saved_templates",['template_category_id'=>$cat_id],['template_category_id'=>'0']);
        ...
        public function update_messenger_template_info()
        {
            ...
            $message="<div class='alert alert-info text-center'><i class='fa fa-check-circle'></i> ".$this->lang->line("Bot template has been updated to database successfully.")."</div><br><a class='btn-block btn btn-outline-info'  href='".base_url('messenger_bot/saved_templates/'.$media_type)."'><i class='fa fa-save'></i> ".$this->lang->line("My Saved Templates")."</a><a target='_BLANK' class='btn-block btn btn-outline-primary' href='".base_url('messenger_bot/export_bot_download/').$id."'><i class='fa fa-file-download'></i> ".$this->lang->line("Download Template")."</a>";
            echo json_encode(array('status'=>'1','message'=>$message));
    ...
    public function otn_template_manager_data()
    {
        ...
        $where_custom .= " AND otn_postback.notification_type = 'otn'";
        ...
        foreach ($info as $key => $value)
        {
            ...
            if(isset($value['flow_type']) && $value['flow_type'] == 'flow')
            {
    ...
    public function otn_subscribers_data()
    {
        ...
        $where_simple['otn_postback.notification_type'] = 'otn';
    ...
    public function rcn_template_manager_data()
    {
        $this->ajax_check();
        $search_value = isset($_POST['search']) ? $_POST['search']['value'] : null;
        $page_id = $this->input->post('page_id',true);
        $display_columns = array("#","CHECKBOX",'id', 'page_name', 'template_name', 'otn_postback_id', 'total_optin_subscriber','rcn_last_sent_time','action');
        $search_columns = ['template_name','otn_postback_id'];
        ...
        foreach ($info as $key => $value)
        {
            $info[$i]["action"] = "<div style='min-width:100px'>";
            $info[$i]["action"] .= "<a target='_BLANK' class='btn btn-circle btn-outline-warning' title='". $this->lang->line("Edit") ."' href='".base_url('visual_flow_builder/edit_builder_data/').$value['visual_flow_campaign_id']."/1'><i class='fas fa-edit'></i></a>&nbsp;";
            $info[$i]["action"] .= "<a class='btn btn-circle btn-outline-primary get_otn_subscribers' page_table_id='".$page_id."' otn_postback_table_id='".$value['id']."' is_iframe='1' title='". $this->lang->line("RCN Subscribers") ."' href='#'><i class='fa fa-users'></i></a>&nbsp;";
            $info[$i]["action"] .= "<div>";
            $info[$i]['flow_type'] = ucfirst($value['flow_type']);
    ...
    public function rcn_clone_template($postback_table_id=0,$iframe='0',$is_default='0')
    {
        check_module_action_access($module_id=335,$actions=1);
    ...
    public function rcn_ajax_delete_template_info()
    {
        if(!check_module_action_access($module_id=335,$actions=3,'check')){
            echo '<div class="alert alert-danger text-center">'.$this->lang->line("You do not have access to perform this action.").'</div>';
            exit();
        }
    ...
    public function get_rcn_postback_dropdown()
    {
        if(!$_POST) exit();
        $page_auto_id=$this->input->post('page_auto_id');// database id

        $postback_id_list = $this->basic->get_data('otn_postback',array('where'=>array('user_id'=>$this->user_id,'page_id'=>$page_auto_id,'notification_type !='=>'otn')),array('id','otn_postback_id','template_name','notification_type'));
        ...
        if(!empty($postback_id_list))
        {
            ...
            {
                $array_key = $value['id'];
                $array_value = $value['otn_postback_id']." (".$value['template_name']." [".$value['notification_type']."])";
                $str .="<option value='{$array_key}'>{$array_value}</option>";
    ...
    public function get_rcn_reply_postback()
    {
        if(!$_POST) exit();
        $page_auto_id=$this->input->post('page_auto_id');// database id
    ...
    public function get_rcn_postback_refresh()
    {
        if(!$_POST) exit();
        $is_from_add_button=$this->input->post('is_from_add_button');
        $page_id=$this->input->post('page_id');// database id
    ...
    public function rcn_subscribers($page_id=0,$iframe='0')
    {
        ...
        $page_list = $this->basic->get_data("facebook_rx_fb_page_info",$where,array('page_name','id'));
        $data['page_info'] = $page_list;
        $data['body'] = 'messenger_tools/rcn_manager/subscribers';
        $data['page_title'] = $this->lang->line('RCN Subscribers');
        $data['page_id'] = $page_id;
        $data['iframe'] = $iframe;
        $this->_viewcontroller($data);
    }

    public function rcn_subscribers_data()
    {
        $this->ajax_check();
        $page_id = $this->input->post('page_table_id',true);
        $postback_id = $this->input->post('postback_id',true);
        $otn_postback_table_id = $this->input->post('otn_postback_table_id',true);
        ...
        $where_simple = array();
        $where_simple['otn_postback.user_id'] = $this->user_id;
        $where_simple['otn_postback.id'] = $otn_postback_table_id;
        $where_simple['otn_postback.notification_type !='] = 'otn';
        if($page_id != '') $where_simple['otn_postback.page_id'] = $page_id;
        if($postback_id != '') $where_simple['otn_postback.otn_postback_id like'] = "%".$postback_id."%";
        $where = array('where'=>$where_simple);
