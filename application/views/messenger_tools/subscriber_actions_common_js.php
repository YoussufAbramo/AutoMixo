<script type="text/javascript">
	...
	function get_subscriber_action_content(id,subscribe_id,page_id,social_media)
	{
		...
		$.ajax({
		  ...
		  data:{id:id,page_id:page_id,subscribe_id:subscribe_id,media_type:social_media},
	...
	$("document").ready(function(){
            ...
            $.ajax({
              ...
              success:function(response){
                 ...
                 if(response.status=='1'){                  
                  ...
                  iziToast.error({title: '',message: '<?php echo $this->lang->line("You do not have access to perform this action."); ?>',position: 'bottomRight'});
                 }
              }
            });
          }
      ...
      $(document).on('click','#save_changes',function(e){
        ...
        var assigned_used_id=$("#subscriber_teams").val();
        ...
        $.ajax({
          ...
          data:{id:id,page_id:page_id,group_id:group_id,campaign_id:campaign_id,social_media:social_media,assigned_used_id:assigned_used_id},
          success:function(response){
             $("#save_changes").removeClass("btn-progress");
             if(response=='1'){
         ...
         if (willDelete) 
         {
             ...
             $.ajax({
               ...
               success:function(response){
                  ...
                  if(response=='1') iziToast.success({title: '',message: '<?php echo $this->lang->line("Subscriber has been deleted successfully."); ?>',position: 'bottomRight'});    
                  else iziToast.error({title: '',message: '<?php echo $this->lang->line("You do not have access to perform this action."); ?>',position: 'bottomRight'});    
