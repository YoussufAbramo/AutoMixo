<?php
$sidebar_user_type = $this->session->userdata('user_type');
?>
    ...
    <ul class="sidebar-menu">
      <?php
        ...
        {          
            ...
            if($single_menu['header_text']!='') echo '<li class="menu-header">'.$this->lang->line($single_menu['header_text']).'</li>';

            if($single_menu['url']=='social_apps/index' && $single_menu['only_member']=='1' && $this->config->item('backup_mode')==='0' && $sidebar_user_type=='Member') continue; // static condition not to

            // team user can not access team features
            if($single_menu['module_access']=='325' && $this->is_manager==1) continue;
            if($single_menu['url']=='integration' && $this->is_manager==1) continue;
            if($single_menu['url']=='integration' && $this->is_manager==1) continue;
            ...
            if(isset($menu_child_1_map[$single_menu['id']]) && count($menu_child_1_map[$single_menu['id']]) > 0)
            {
              ...
              {                  
                  ...
                  if($sidebar_user_type == 'Admin' && $this->session->userdata('license_type') != 'double' && in_array($single_child_menu['url'], $admin_double_level2)) continue;

                  if(($only_admin2 == '1' && $sidebar_user_type == 'Member') || ($only_member2 == '1' && $sidebar_user_type == 'Admin')) 
                  continue;

                  // team user can not access
                  if($only_admin2 == '1' && $this->is_manager == 1) 
                  continue;
                  ...
                  if(isset($menu_child_2_map[$single_child_menu['id']]) && count($menu_child_2_map[$single_child_menu['id']]) > 0)
                  {
                    ...
                    { 
                      ...
                      // team user can not access
                      if($only_admin3 == '1' && $this->is_manager == 1) 
                      continue;
            ...
            if($only_admin == '1') 
            {
              if($sidebar_user_type == 'Admin' &&  $this->is_manager != 1) 
              echo $menu_html;
            }
            ...
            {
              if($sidebar_user_type=="Admin" || empty($module_access) || count(array_intersect($this->module_access, $module_access))>0 ) 
              echo $menu_html;
            }             
        }

        if($this->session->userdata('license_type') == 'double' && $sidebar_user_type=='Member' && $this->is_manager!=1)
        {
          echo'
...
(
  ...
  base_url("comment_automation/all_auto_comment_report?media_type=".$this->using_media_type)=>base_url("comment_automation/comment_section_report?media_type=".$this->using_media_type),
  base_url("comment_automation/all_auto_comment_report/0/0")=>base_url("instagram_reply/reports"),
  base_url("comment_automation/all_auto_reply_report?media_type=".$this->using_media_type)=>base_url("comment_automation/comment_section_report?media_type=".$this->using_media_type),
  base_url("comment_reply_enhancers/bulk_tag_campaign_list?media_type=".$this->using_media_type)=>base_url("comment_automation/comment_section_report?media_type=".$this->using_media_type),
  base_url("comment_reply_enhancers/bulk_comment_reply_campaign_list?media_type=".$this->using_media_type)=>base_url("comment_automation/comment_section_report?media_type=".$this->using_media_type),
  base_url("comment_reply_enhancers/all_response_report?media_type=".$this->using_media_type)=>base_url("comment_automation/comment_section_report?media_type=".$this->using_media_type),
  base_url("comment_reply_enhancers/all_like_share_report?media_type=".$this->using_media_type)=>base_url("comment_automation/comment_section_report?media_type=".$this->using_media_type),
